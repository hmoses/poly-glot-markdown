 ...
 * Each stage appends to the canvas (no wiping previous stages).
 * Click step cards to jump. Click canvas to pause/resume.
 */
(function () {
    ...
    var playing = false;
    var paused  = false;
    var cancelled = false; // full cancel on reset
    ...
    function typeSmooth(el, text, framesPerChar) {
        ...
        return new Promise(function(resolve) {
            var pos = 0, frame = 0;
            function tick() {
                if (cancelled) { el.innerHTML = '<pre style="margin:0;white-space:pre-wrap;color:#a5f3fc;font-size:12px;font-family:Fira Code,monospace;line-height:1.6">' + esc(text) + '</pre>'; resolve(); return; }
    ...
    function setActive(n) {
        stages.forEach(function(s, i) {
            if (!s) return;
            s.style.opacity = i < n ? '0.5' : i === n ? '1' : '0.5';
    ...
    /* ── Stage 1: Show formatted document ── */
    async function showStage1() {
        if (cancelled) return;
        setActive(0);
        canvas.innerHTML = '';
        var panel = document.createElement('div');
        panel.className = 'pl-demo-panel';
        panel.id = 'plPanel1';
        ...
        // Show chunks derived from the doc
        var arrow = document.createElement('div');
        arrow.className = 'pl-demo-arrow-down';
        arrow.textContent = '↓ Split by chunk boundaries';
        canvas.appendChild(arrow);
        await sleep(800);
        if (cancelled) return;

        var chunks = document.createElement('div');
        chunks.className = 'pl-demo-chunks';
        chunks.innerHTML = '<div class="pl-demo-chunk"><strong>Chunk 1:</strong> Frontmatter (metadata)</div><div class="pl-demo-chunk"><strong>Chunk 2:</strong> Getting Your API Key</div><div class="pl-demo-chunk"><strong>Chunk 3:</strong> Rate Limits</div>';
        canvas.appendChild(chunks);
        await sleep(2500);
    }
    ...
    async function showStage2() {
        ...
        var arrow = document.createElement('div');
        arrow.className = 'pl-demo-arrow-down';
        arrow.textContent = '↓ Embed each chunk';
        canvas.appendChild(arrow);
        await sleep(600);
        if (cancelled) return;
        ...
        var chunkNames = ['Frontmatter', 'Getting Your API Key', 'Rate Limits'];
        for (var i = 0; i < chunkNames.length; i++) {
            if (cancelled) return;
            await sleep(1200);
            if (cancelled) return;
            var vec = Array.from({length: 8}, function(){ return (Math.random()*2-1).toFixed(3); });
            grid.innerHTML += '<div class="pl-demo-embed-row"><div class="pl-demo-embed-chunk">' + chunkNames[i] + '</div><div class="pl-demo-embed-arrow">→</div><div class="pl-demo-embed-vec">[' + vec.join(', ') + '…]</div><div class="pl-demo-embed-ok">✅</div></div>';
        }
        if (cancelled) return;
        await sleep(800);
    ...
    async function showStage3() {
        ...
        var arrow = document.createElement('div');
        arrow.className = 'pl-demo-arrow-down';
        arrow.textContent = '↓ User asks a question';
        canvas.appendChild(arrow);
        await sleep(600);
        if (cancelled) return;
        ...
        searchDiv.innerHTML = '<div class="pl-demo-searching">Computing cosine similarity across 3 chunks…</div>';
        if (cancelled) return;
        await sleep(2000);
        if (cancelled) return;

        searchDiv.innerHTML = '<div class="pl-demo-search-result pl-demo-match-high"><span class="pl-demo-match-score">94.2%</span><span class="pl-demo-match-title">Rate Limits</span><span class="pl-demo-match-preview">Free tier: 100 req/min. Pro tier: 10,000 req/min.</span></div>'
    ...
    async function showStage4() {
        ...
        var arrow = document.createElement('div');
        arrow.className = 'pl-demo-arrow-down';
        arrow.textContent = '↓ Send top chunks + question to LLM';
        canvas.appendChild(arrow);
        await sleep(600);
        if (cancelled) return;
        ...
        var answerText = 'The free tier allows 100 requests per minute.\nIf you need higher throughput, the Pro tier\nsupports up to 10,000 requests per minute.\n\n[Source: Rate Limits, similarity: 94.2%]';
        await typeSmooth(document.getElementById('plAnswerType'), answerText, 3);
        if (cancelled) return;
        await sleep(800);

        var sources = document.createElement('div');
        sources.className = 'pl-demo-answer-sources';
        sources.innerHTML = '<span style="font-size:11px;color:#64748b;font-weight:700;text-transform:uppercase;letter-spacing:.05em">Sources:</span><span class="pl-demo-source-tag">📄 Rate Limits — 94.2%</span><span class="pl-demo-source-tag">📄 Getting Your API Key — 61.8%</span>';
        canvas.appendChild(sources);
    }
    ...
    async function runDemo() {
        ...
        canvas.insertAdjacentHTML('beforeend', '<div id="plPauseBanner" style="text-align:center;padding:10px;font-size:13px;color:#34d399;font-weight:600;background:rgba(16,185,129,0.08);border-radius:6px;margin-top:12px">✅ Demo complete — click Reset to replay</div>');
    }
